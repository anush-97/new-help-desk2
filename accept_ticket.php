...
if (isset($_POST['ticket_id'])) {
    ...
    $sql = "UPDATE new_ticket SET status = 'ACCEPT' WHERE id = ?";
    ...
    if ($stmt->execute()) {
        ...
        echo json_encode(["status" => "success", "message" => "Ticket accepted successfully."]);
    } else {
        // Return an error message
        echo json_encode(["status" => "error", "message" => "Error accepting ticket: " . $conn->error]);
    }
